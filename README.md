...
1. **管理图谱**：顶部菜单栏「图谱」可新建/删除多个关系图谱（每套图谱是一套独立的人物与关系），信息栏下拉切换当前图谱、文本框直接改名。
2. **新增成员**：「成员」菜单 →「新增成员」，在右侧详情面板填写姓名/性别/生日/备注。
3. **设置关系**：在详情面板为成员选择「父亲 / 母亲 / 配偶」。兄弟姐妹与子女会**按父/母关系自动推导**（同父/同母即视为兄弟姐妹），无需手动录入。
4. **指定「我」**：选中某人后「成员」菜单 →「设为『我』」。
5. **查看称谓**：每个人的称谓直接显示在中间图谱的节点卡片上；图谱支持**拖拽平移、滚轮缩放、点击节点选中、拖拽移动成员卡片**，选中成员时会**高亮加粗与其直接相连的连线**（其余连线淡出），点击图谱空白处取消选中；拖拽后可用信息栏「重置布局」恢复自动布局与视角。
6. **调整称谓（地区/方言）**：「图谱」菜单 →「称谓规则」打开窗口，切换内置方言预设（普通话 / 粤语），或「复制为自定义」后修改称谓文本、添加/删除规则；修改立即应用到当前图谱。
7. **数据传递**：「文件」菜单提供导出 JSON 文件、复制 JSON、导入 JSON 文件、粘贴导入。

左侧成员列表按拼音排序，可通过其上方标题栏的「隐藏/显示」按钮折叠/展开；右侧详情面板在没有选中成员时自动收起、不占空间。左右分隔条均可拖动调整宽度。
